...
int main()
{
	...
	for(i=1;i<(int)outdoor_rightorder.size();i++)
	{
		...
		double distance_between=norm(outdoor_mask[outdoor_rightorder[i][4].outdoor_order].image,
			outdoor_mask[outdoor_rightorder[i-1][j-5].outdoor_order].image,CV_L1);
